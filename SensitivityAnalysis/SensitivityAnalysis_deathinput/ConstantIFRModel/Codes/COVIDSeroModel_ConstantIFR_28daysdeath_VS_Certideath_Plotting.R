...
cert_gamma_NorthWest <- rstan::extract(CertificateSeroModelConstantIFR)$gamma_NorthWest
cert_gamma_SouthEast <- rstan::extract(CertificateSeroModelConstantIFR)$gamma_SouthEast
cert_gamma_SouthWest <- rstan::extract(CertificateSeroModelConstantIFR)$gamma_SouthWest
...
data<-data.frame(class=factor(rep(c("Model 10","Model 2"),each=length(beta))),
                 ...
                 para_SouthEast=c(gamma_SouthEast,cert_gamma_SouthEast),
                 para_NorthWest=c(gamma_NorthWest,cert_gamma_NorthWest),
                 para_SouthWest=c(gamma_SouthWest,cert_gamma_SouthWest),
                 para_Midlands=c(gamma_Midlands,cert_gamma_Midlands),
                 para_EastofEngland=c(gamma_EastofEngland,cert_gamma_EastofEngland))
them<-theme(
...
for (i in 1:sim) {
  x_London[i,]<-rnbinom(rep(1,n_days_London), size= 100, mu=cumsum(exp(beta[i]*t_London)*(1-gamma_London[i])/gamma_London[i]*daily_death_London28)/(exp(beta[i]*t_London)))/(P0_London-cumul_death_London28)
  x_London_cert[i,]<-rnbinom(rep(1,n_days_London), size= 100, mu=cumsum(exp(cert_beta[i]*t_London)*(1-cert_gamma_London[i])/cert_gamma_London[i]*daily_death_London)/(exp(cert_beta[i]*t_London)))/(P0_London-cumul_death_London)
  
  epsilon_London[i,]<-cumsum((1-gamma_London[i])/gamma_London[i]*daily_death_London28)/(P0_London-cumul_death_London28)
  epsilon_London_cert[i,]<-cumsum((1-cert_gamma_London[i])/cert_gamma_London[i]*daily_death_London)/(P0_London-cumul_death_London)
  
  kft_London[i,]<-gamma_London[i]
  kft_London[i,]<-cert_gamma_London[i]
...
for (i in 1:sim) {
  x_NorthWest[i,]<-rnbinom(rep(1,n_days_NorthWest), size= 100, mu=cumsum(exp(beta[i]*t_NorthWest)*(1-gamma_NorthWest[i])/gamma_NorthWest[i]*daily_death_NorthWest28)/(exp(beta[i]*t_NorthWest)))/(P0_NorthWest-cumul_death_NorthWest28)
  x_NorthWest_cert[i,]<-rnbinom(rep(1,n_days_NorthWest), size= 100, mu=cumsum(exp(cert_beta[i]*t_NorthWest)*(1-cert_gamma_NorthWest[i])/cert_gamma_NorthWest[i]*daily_death_NorthWest)/(exp(cert_beta[i]*t_NorthWest)))/(P0_NorthWest-cumul_death_NorthWest)
  
  epsilon_NorthWest[i,]<-cumsum((1-gamma_NorthWest[i])/gamma_NorthWest[i]*daily_death_NorthWest28)/(P0_NorthWest-cumul_death_NorthWest28)
  epsilon_NorthWest_cert[i,]<-cumsum((1-cert_gamma_NorthWest[i])/cert_gamma_NorthWest[i]*daily_death_NorthWest)/(P0_NorthWest-cumul_death_NorthWest)
  
  kft_NorthWest[i,]<-gamma_NorthWest[i]
  kft_NorthWest[i,]<-cert_gamma_NorthWest[i]
...
for (i in 1:sim) {
  x_SouthWest[i,]<-rnbinom(rep(1,n_days_SouthWest), size= 100, mu=cumsum(exp(beta[i]*t_SouthWest)*(1-gamma_SouthWest[i])/gamma_SouthWest[i]*daily_death_SouthWest28)/(exp(beta[i]*t_SouthWest)))/(P0_SouthWest-cumul_death_SouthWest28)
  x_SouthWest_cert[i,]<-rnbinom(rep(1,n_days_SouthWest), size= 100, mu=cumsum(exp(cert_beta[i]*t_SouthWest)*(1-cert_gamma_SouthWest[i])/cert_gamma_SouthWest[i]*daily_death_SouthWest)/(exp(cert_beta[i]*t_SouthWest)))/(P0_SouthWest-cumul_death_SouthWest)
  
  epsilon_SouthWest[i,]<-cumsum((1-gamma_SouthWest[i])/gamma_SouthWest[i]*daily_death_SouthWest28)/(P0_SouthWest-cumul_death_SouthWest28)
  epsilon_SouthWest_cert[i,]<-cumsum((1-cert_gamma_SouthWest[i])/cert_gamma_SouthWest[i]*daily_death_SouthWest)/(P0_SouthWest-cumul_death_SouthWest)
  
  kft_SouthWest[i,]<-gamma_SouthWest[i]
  kft_SouthWest[i,]<-cert_gamma_SouthWest[i]
...
for (i in 1:sim) {
  x_SouthEast[i,]<-rnbinom(rep(1,n_days_SouthEast), size= 100, mu=cumsum(exp(beta[i]*t_SouthEast)*(1-gamma_SouthEast[i])/gamma_SouthEast[i]*daily_death_SouthEast28)/(exp(beta[i]*t_SouthEast)))/(P0_SouthEast-cumul_death_SouthEast28)
  x_SouthEast_cert[i,]<-rnbinom(rep(1,n_days_SouthEast), size= 100, mu=cumsum(exp(cert_beta[i]*t_SouthEast)*(1-cert_gamma_SouthEast[i])/cert_gamma_SouthEast[i]*daily_death_SouthEast)/(exp(cert_beta[i]*t_SouthEast)))/(P0_SouthEast-cumul_death_SouthEast)
  
  epsilon_SouthEast[i,]<-cumsum((1-gamma_SouthEast[i])/gamma_SouthEast[i]*daily_death_SouthEast28)/(P0_SouthEast-cumul_death_SouthEast28)
  epsilon_SouthEast_cert[i,]<-cumsum((1-cert_gamma_SouthEast[i])/cert_gamma_SouthEast[i]*daily_death_SouthEast)/(P0_SouthEast-cumul_death_SouthEast)
  
  kft_SouthEast[i,]<-gamma_SouthEast[i]
  kft_SouthEast[i,]<-cert_gamma_SouthEast[i]
...
for (i in 1:sim) {
  x_EastofEngland[i,]<-rnbinom(rep(1,n_days_EastofEngland), size= 100, mu=cumsum(exp(beta[i]*t_EastofEngland)*(1-gamma_EastofEngland[i])/gamma_EastofEngland[i]*daily_death_EastofEngland28)/(exp(beta[i]*t_EastofEngland)))/(P0_EastofEngland-cumul_death_EastofEngland28)
  x_EastofEngland_cert[i,]<-rnbinom(rep(1,n_days_EastofEngland), size= 100, mu=cumsum(exp(cert_beta[i]*t_EastofEngland)*(1-cert_gamma_EastofEngland[i])/cert_gamma_EastofEngland[i]*daily_death_EastofEngland)/(exp(cert_beta[i]*t_EastofEngland)))/(P0_EastofEngland-cumul_death_EastofEngland)
  
  epsilon_EastofEngland[i,]<-cumsum((1-gamma_EastofEngland[i])/gamma_EastofEngland[i]*daily_death_EastofEngland28)/(P0_EastofEngland-cumul_death_EastofEngland28)
  epsilon_EastofEngland_cert[i,]<-cumsum((1-cert_gamma_EastofEngland[i])/cert_gamma_EastofEngland[i]*daily_death_EastofEngland)/(P0_EastofEngland-cumul_death_EastofEngland)
  
  kft_EastofEngland[i,]<-gamma_EastofEngland[i]
  kft_EastofEngland[i,]<-cert_gamma_EastofEngland[i]
...
for (i in 1:sim) {
  x_Midlands[i,]<-rnbinom(rep(1,n_days_Midlands), size= 100, mu=cumsum(exp(beta[i]*t_Midlands)*(1-gamma_Midlands[i])/gamma_Midlands[i]*daily_death_Midlands28)/(exp(beta[i]*t_Midlands)))/(P0_Midlands-cumul_death_Midlands28)
  x_Midlands_cert[i,]<-rnbinom(rep(1,n_days_Midlands), size= 100, mu=cumsum(exp(cert_beta[i]*t_Midlands)*(1-cert_gamma_Midlands[i])/cert_gamma_Midlands[i]*daily_death_Midlands)/(exp(cert_beta[i]*t_Midlands)))/(P0_Midlands-cumul_death_Midlands)
  
  epsilon_Midlands[i,]<-cumsum((1-gamma_Midlands[i])/gamma_Midlands[i]*daily_death_Midlands28)/(P0_Midlands-cumul_death_Midlands28)
  epsilon_Midlands_cert[i,]<-cumsum((1-cert_gamma_Midlands[i])/cert_gamma_Midlands[i]*daily_death_Midlands)/(P0_Midlands-cumul_death_Midlands)
  
  kft_Midlands[i,]<-gamma_Midlands[i]
  kft_Midlands[i,]<-cert_gamma_Midlands[i]
...
for (i in 1:sim) {
  x_NorthEastYorkshireHumber[i,]<-rnbinom(rep(1,n_days_NorthEastYorkshireHumber), size= 100, mu=cumsum(exp(beta[i]*t_NorthEastYorkshireHumber)*(1-gamma_NorthEastYorkshireHumber[i])/gamma_NorthEastYorkshireHumber[i]*daily_death_NorthEastYorkshireHumber28)/(exp(beta[i]*t_NorthEastYorkshireHumber)))/(P0_NorthEastYorkshireHumber-cumul_death_NorthEastYorkshireHumber28)
  x_NorthEastYorkshireHumber_cert[i,]<-rnbinom(rep(1,n_days_NorthEastYorkshireHumber), size= 100, mu=cumsum(exp(cert_beta[i]*t_NorthEastYorkshireHumber)*(1-cert_gamma_NorthEastYorkshireHumber[i])/cert_gamma_NorthEastYorkshireHumber[i]*daily_death_NorthEastYorkshireHumber)/(exp(cert_beta[i]*t_NorthEastYorkshireHumber)))/(P0_NorthEastYorkshireHumber-cumul_death_NorthEastYorkshireHumber)
  
  epsilon_NorthEastYorkshireHumber[i,]<-cumsum((1-gamma_NorthEastYorkshireHumber[i])/gamma_NorthEastYorkshireHumber[i]*daily_death_NorthEastYorkshireHumber28)/(P0_NorthEastYorkshireHumber-cumul_death_NorthEastYorkshireHumber28)
  epsilon_NorthEastYorkshireHumber_cert[i,]<-cumsum((1-cert_gamma_NorthEastYorkshireHumber[i])/cert_gamma_NorthEastYorkshireHumber[i]*daily_death_NorthEastYorkshireHumber)/(P0_NorthEastYorkshireHumber-cumul_death_NorthEastYorkshireHumber)
  
  kft_NorthEastYorkshireHumber[i,]<-gamma_NorthEastYorkshireHumber[i]
  kft_NorthEastYorkshireHumber[i,]<-cert_gamma_NorthEastYorkshireHumber[i]
...
tiff(file=paste(folder,"/ParameterEstimation_deathInput1.tiff", sep = ""),
