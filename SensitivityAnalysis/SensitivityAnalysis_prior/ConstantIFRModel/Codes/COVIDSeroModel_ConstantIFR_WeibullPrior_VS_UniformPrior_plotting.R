...
ConstantSeroModelConstantIFR_Weibull <- readRDS(paste(folder,"/ConstantSeroModelConstantIFR_Weibull.rds", sep = ""))
SeroModelConstantIFR_Uniform <- readRDS(paste(folder,"/SeroModelConstantIFR.rds", sep = ""))
...
data<-data.frame(class=factor(rep(c("Scenario 2","Scenario 12"),each=length(beta_Uniform))),
